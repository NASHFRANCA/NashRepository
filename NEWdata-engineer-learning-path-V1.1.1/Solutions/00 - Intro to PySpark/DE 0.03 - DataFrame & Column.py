...
distinct_users_df = events_df.dropDuplicates(["user_id"]) #only drops duplocates from column in the square brackets. Square brackets are needed 
display(distinct_users_df)

# COMMAND ----------

#below is not useful as only shows unique devices but I wanted to mess around with code. 

distinct_devices_df = events_df.dropDuplicates(["device"])
display(distinct_devices_df)
