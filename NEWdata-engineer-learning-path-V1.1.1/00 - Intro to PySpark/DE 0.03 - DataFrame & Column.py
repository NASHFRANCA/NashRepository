...
result = col("ecommerce.purchase_revenue_in_usd") + col("ecommerce.total_item_quantity")
...
#withColumn creates new column based on existing one. in this case purchase revenue and average purchase rev are new
#cast() – cast() is a function from Column class that is used to convert the column into the other datatype. that's why it's in withcolumn
...
my_df = (events_df
     .filter(col("ecommerce.purchase_revenue_in_usd").isNotNull())
     .filter(col("ecommerce.total_item_quality").isNotNull())
     .withColumn("new_Revenue_Column", (col("ecommerce.purchase_revenue_in_usd") * (col("ecommerce.total_item_quality")))
    )
...
new_df = (events_df
    .select('device', 'ecommerce.purchase_revenue_in_usd')
    .where(col('ecommerce.purchase_revenue_in_usd') > 1900))
...
#"in" 	Returns True if a sequence with the specified value is present in the object. object here being device.
# "as"  create an alias. in this case the alias is "apple user"
