...
#Here we are using python syntax to sort the dataframe events_df

mac_df = events_df[ (events_df['device']) == 'macOS'].sort('event_timestamp')
display(mac_df)

# COMMAND ----------

 # here we are using the spark DataFrame API's  programming approach for data manipulation in PySpark
# I suppose we don't need to call out the spark at the beginning? 
...
#.take returns an ARRAY of the n number rows specified
rows = mac_df.take(5)
display(rows)
