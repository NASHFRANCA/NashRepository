...
#Simpler method to convert from unix to timestamp
from pyspark.sql.functions import from_unixtime

timestamp_df = df.withColumn("timestamp", from_unixtime(col("timestamp")/1e6).cast("timestamp"))
display(timestamp_df)
...
date_df = timestamp_df.withColumn("date", F.to_date(col("timestamp")))
display(date_df)
